 ...
 reinterpret_cast<GLvoid*>(offsetof(cls,mbr)))
	...
	*/
	bool FbxLoader::Load(const char*filename) {
		...
		if (!fbxScene) {
			std::cerr << "ERROR:" << filename << "の読み込みに失敗(FbxSceneの作成に失敗)" <<
				std::endl;
			return false;
		}
		else {
	...
	*/
	bool FbxLoader::LoadMesh(FbxNode*fbxNode) {
		...
		if (!fbxMesh->IsTriangleMesh()) {
			std::cerr << "WARNIG:" << mesh.name << "には三角形以外の面が含まれています" << std::endl;
		...
		//頂点がどのマテリアルに属するかを示すマテリアルインデックスを取得する
		...
		for (int polygonIndex = 0; polygonIndex < polygonCount; ++polygonIndex) {
			for (int pos = 0; pos < 3; ++pos) {
				...
				//頂点に対応する仮マテリアルに頂点データとインデックスデータを追加する
